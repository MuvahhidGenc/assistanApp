...
class V4Settings:
    ...
    def _build_advanced(self) -> None:
        ...
        self._label("Sistem Günlükleri")
        try:
            from hermes.config.paths import app_log_path
            _log_path_hint = str(app_log_path())
        except Exception:
            _log_path_hint = "%LOCALAPPDATA%\\HermesClient\\logs\\app.log"
        self._hint(_log_path_hint)
        btn_logs = ctk.CTkButton(
            self._cur,
            text="LOG KAYDINI GÖRÜNTÜLE",
            width=220,
            height=38,
            fg_color=BUTTON,
            hover_color=BUTTON_HOVER,
        ...
        btn_logs.grid(row=self._row, column=0, columnspan=2, sticky="w", padx=16, pady=(0, 16))
        self._row += 1
    ...
    def _open_logs(self) -> None:
        try:
            ...
            LogsWindow(self.root.winfo_toplevel()).show()
        except Exception as exc:  # pragma: no cover
            try:
                self._show_banner(f"Log görüntüleyici açılamadı: {exc}", NEON_MAGENTA)
            except Exception:
                pass
