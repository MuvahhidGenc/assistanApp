...
class V4AppShell(ctk.CTk):
    def __init__(
        ...
        **kwargs,
        ...
        if worker is not None and hasattr(worker, "send_message") and callable(worker.send_message):
            self._on_send_message = worker.send_message
        else:
        ...
        if worker is not None:
            try:
                bind_store_events(worker, self.store)
            ...
            try:
                state_msgs = getattr(getattr(worker, "state", None), "messages", None) or []
