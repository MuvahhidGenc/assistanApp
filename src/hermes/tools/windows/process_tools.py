...
class ListProcessesTool(BaseTool):
    ...
    async def execute(
        self, limit: int = 20, sort_by: str = "cpu", **kwargs: Any
    ) -> ToolExecutionResult:
        try:
            limit = min(max(limit, 1), 100)
            sort_prop = "CPU" if sort_by.lower() == "cpu" else "WorkingSet"
            data = await run_powershell_json(
                f"Get-Process | Sort-Object {sort_prop} -Descending | Select-Object -First {limit} "
                "Id,ProcessName,CPU,@{N='MemoryMB';E={[math]::Round($_.WorkingSet64/1MB,1)}},Path"
            )
...
class ListServicesTool(BaseTool):
    ...
    async def execute(
        self, filter_name: str = "", status: str = "", limit: int = 50, **kwargs: Any
    ) -> ToolExecutionResult:
        try:
            ...
            script_parts.append(f"Select-Object -First {limit} Name,DisplayName,Status,StartType")
