...
# JSON Schema fragments for tools with parameters (empty object otherwise).
...
_LOCAL_TOOL_PATTERN = re.compile(r"LOCAL_TOOL", re.IGNORECASE)
...
def format_tool_result_message(
    ...
    *,
    ...
    message = (
        ...
        "Yukaridaki yerel tool sonucunu kullanarak kullaniciya kisa ve anlasilir "
        "Turkce ozet ver. Ham JSON tekrarlama."
    )
    ...
    if tool_name == "open_app" and url_hint and result.success:
        message = (
            f"TOOL_RESULT {json.dumps(payload, ensure_ascii=False, default=str)}\n\n"
            f"Devam adimi: Kullanici ayrica {url_hint} adresine gitmek istedi. "
            "Ozeti henuz verme — once LOCAL_TOOL "
            f'{{"name": "open_url", "arguments": {{"url": "{url_hint}"}}}} cagir.'
        )
    return message
