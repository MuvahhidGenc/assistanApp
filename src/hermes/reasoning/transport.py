...
def _build_system_prompt(prompt: ReasoningPrompt) -> str:
    ...
    return (
        ...
        "  5. YEREL dosya/klasor islemlerinde terminal.execute (run_command) "
        "KULLANMA — bunun icin filesystem.write (create_folder/write_file), "
        "filesystem.read, filesystem.copy, filesystem.move yetenekleri var. "
        "run_command yalnizca PowerShell sistem yonetimi (servis, regedit, "
        "network, kurulum sonrasi) icindir ve yuksek riskli oldugundan onay "
        "ister; gereksiz onay beklemeleri kullaniciyi zorlar.\n"
        "\n"
        "Ornekler:\n"
