...
from hermes.config.paths import bundled_config_path, ensure_user_dirs, portable_config_path, user_config_path
...
def bootstrap_user_config() -> Path | None:
    ...
    payload = _load_base_yaml()
    payload.setdefault("server", {})
    if not payload["server"].get("url"):
        payload["server"]["url"] = "http://50.6.226.228:8642"
    if not payload["server"].get("model"):
        payload["server"]["model"] = normalize_model("hermes-agent")
