...
def test_attack_5_secret_in_tool_arguments_is_redacted_in_audit():
    ...
    tmp = Path("/tmp/audit_attack_5.log")
    if tmp.exists():
        tmp.unlink()
    al = AuditLogger(tmp)
    al.log(
...
def test_attack_8_path_traversal_in_arguments_does_not_change_resolution():
    ...
    tmp = Path("/tmp/audit_attack_8.log")
    if tmp.exists():
        tmp.unlink()
    al = AuditLogger(tmp)
    al.log(
...
def test_attack_9_denial_audit_does_not_echo_secrets():
    ...
    async def go():
        tmp = Path("/tmp/audit_attack_9.log")
        if tmp.exists():
            tmp.unlink()
        al = AuditLogger(tmp)
        am = ApprovalManager()
...
def test_attack_10_recovery_budget_cannot_be_infinite():
    ...
    work = Path("/tmp/recovery_attack_10.jsonl")
    if work.exists():
        work.unlink()
