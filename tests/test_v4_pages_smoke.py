...
def test_v4_page_uses_only_real_store_keys_no_fake_state(mod_name, cls_name):
    ...
    class GuardedStore(V4UIStore):
        def get_snapshot(self):
            ...
            # no extra keys beyond canonical + possible private impl keys:
            extra = set(snap.keys()) - real_keys
            assert not extra, f"store snapshot contains extra (non-canonical) keys: {extra}"
            return snap
